...
1. 修改配置
   ...
   - `docker-compose.yml`
     ...
     ```sh
     $ cp docker-compose.sample.yml docker-compose.test.yml 
     # 选择对应的`php`和`mysql`版本,开启需要的服务 
     ```
   ...
   - `env`
     ...
     $ vim .env.test (需要修改以下几项)
       LARAVEL_ENV  # 项目运行依赖环境变量
       NGINX_HTTP_HOST_PORT # Nginx http端口
       NGINX_HTTPS_HOST_PORT # Nginx https端口
       NGINX_CONFD_DIR  #vhost目录
       NGINX_FASTCGI_PARAMS  # Nginx fastcgi变量
       MYSQL5_HOST_PORT  # MySQL 端口
       MYSQL5_ROOT_PASSWORD # MySQL root用户初始密码
       REDIS_HOST_PORT # Redis 端口
       REDISMYADMIN_HOST_PORT # PhpRedisAdmin端口 
     ```

   - `conf.d`

     ```sh
     $ cp -r services/nginx/conf.d.sample services/nginx/conf.d.test
     $ cd services/nginx/conf.d.test
     $ vim niuchen.test.conf (需要修改以下几项)
       listen         # 监听端口号
       server_name    # 域名
       root           # 项目路径
       access_log     # 错误日志
       error_log      # 错误日志
       try_files $uri $uri/ /index.php?$args; # 重定向
       fastcgi_pass   # 监听PHP地址
       ssl_certificate # ssl证书路径 (PEM格式)
       ssl_certificate_key # ssl证书密钥(KEY格式)
     $ cd -                                                                          #返回根目录
     ```
   ...
   - `fastcgi_params`
     ...
     ```sh 
     $ cp services/nginx/fastcgi_params.sample services/nginx/fastcgi_params.test
     $ vim services/nginx/fastcgi_params.test (需要修改以下几项)
       fastcgi_param APP_ENV  test  # fastcgi环境变量
     ```
...
2. 启动服务
   ...
   ```
   $ \cp -f .env.test .env
   $ \cp -f docker-compose.test.yml  docker-compose.yml 
   $ docker-compose up -d
   ```
...
3. 部署项目
   ...
   ```sh
   $ cd www                                                             # 进入www目录
   $ git clone ssh://git@119.29.40.230:222/root/niuchen.git             # 克隆纽臣项目
   $ chown -R 1000:1000 niuchen                                         # 修改用户权限
   $ docker exec -it php /bin/sh -c "cd niuchen && composer install"    # 安装composer软件包 
   ```
...
- Pcntl
  ...
  ```sh
  $ docker exec -it pcntl /bin/sh
  $ install-php-extensions pcntl
  $ php -m | grep pcntl 
  ```
4. 批量执行命令
```
$ docker exec -it php /bin/sh -c "cd niuchen && pwd"
```
